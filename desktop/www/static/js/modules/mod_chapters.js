...
(function() {
    ...
    function renderList() {
        ...
        sorted.forEach((ch, idx) => {
            ...
            html += `<button class="btn-tiny" onclick="ChaptersModule.reviewChapter('${ch.id}')">审查</button>`;
    ...
    function showAddChapter() {
        showModal('添加章节', `
            <div style="display:flex;flex-direction:column;gap:12px;">
                ...
                <div><label>正文</label><textarea id="ch-content" class="modal-input" rows="6" placeholder="章节正文（也可在「章节正文审查」模块内编辑）..."></textarea></div>
            </div>
        `, [
    ...
    function showEditChapter(chId) {
        ...
        showModal('编辑章节', `
            <div style="display:flex;flex-direction:column;gap:12px;">
                ...
                <div><label>正文</label><textarea id="ch-content" class="modal-input" rows="8">${escapeHtml(ch.content || '')}</textarea></div>
            </div>
        `, [
            { text: '取消', class: 'btn-secondary', action: () => closeModal() },
            { text: '保存', class: 'btn-primary', action: async () => {
                ...
                const newContent = document.getElementById('ch-content').value;
                // 正文变化则清空旧审查缓存
    ...
    window.ChaptersModule = { loadData, refreshView, showAddChapter, showEditChapter, deleteChapter, reviewChapter, exportData: () => { if (typeof exportModule === 'function') exportModule('chapters'); } };
