...
(function() {
    ...
    function collectAllIds() {
        ...
        if (data.character && data.character.name) {
            items.push({ module: '角色信息', moduleId: 'character', icon: 'user', id: data.character.id || 'char_main', name: data.character.name });
        }
        ...
        if (data.currency && typeof data.currency === 'object') {
            for (const [key, value] of Object.entries(data.currency)) {
                ...
                items.push({ module: '货币', moduleId: 'currency', icon: 'coin', id: key, name: typeInfo.name || key });
            }
        }
        ...
        if (Array.isArray(inv)) {
            inv.forEach(item => {
                items.push({ module: '背包', moduleId: 'inventory', icon: 'backpack', id: item.id || '-', name: item.name || '未命名' });
            });
        } else if (inv && typeof inv === 'object') {
            for (const [key, value] of Object.entries(inv)) {
                if (value && typeof value === 'object') {
                    items.push({ module: '背包', moduleId: 'inventory', icon: 'backpack', id: value.id || key, name: value.name || '未命名' });
                }
            }
        }
        ...
        if (Array.isArray(lib)) {
            lib.forEach(item => {
                items.push({ module: '物品库', moduleId: 'item-library', icon: 'shop', id: item.id || '-', name: item.name || '未命名' });
            });
        } else if (lib && typeof lib === 'object') {
            for (const [key, value] of Object.entries(lib)) {
                const item = value && typeof value === 'object' ? value : {};
                items.push({ module: '物品库', moduleId: 'item-library', icon: 'shop', id: item.id || key, name: item.name || '未命名' });
            }
        }
        ...
        if (Array.isArray(data.equipmentSlots)) {
            data.equipmentSlots.forEach(slot => {
                items.push({ module: '装备', moduleId: 'equipment', icon: 'sword', id: slot.id || '-', name: slot.name || '未命名' });
                if (slot.item && slot.item.id) {
                    items.push({ module: '装备(物品)', moduleId: 'equipment', icon: 'sword', id: slot.item.id, name: slot.item.name || '未命名' });
                }
            });
        }
        ...
        if (Array.isArray(data.quests)) {
            data.quests.forEach(q => {
                items.push({ module: '任务', moduleId: 'quests', icon: 'scroll', id: q.id || '-', name: q.name || q.title || '未命名' });
            });
        }
        ...
        if (Array.isArray(data.skills)) {
            data.skills.forEach(s => {
                items.push({ module: '技能', moduleId: 'skills', icon: 'spark', id: s.id || '-', name: s.name || '未命名' });
            });
        }
        ...
        if (Array.isArray(data.storyMarks)) {
            data.storyMarks.forEach(m => {
                items.push({ module: '剧情', moduleId: 'story', icon: 'book', id: m.id || m.mark_id || '-', name: m.title || m.description || '未命名' });
            });
        }
        ...
        if (Array.isArray(data.foreshadowing)) {
            data.foreshadowing.forEach(f => {
                items.push({ module: '剧情(伏笔)', moduleId: 'story', icon: 'book', id: f.id || '-', name: f.title || '未命名' });
            });
        }
        ...
        if (Array.isArray(v183.locations)) {
            v183.locations.forEach(loc => {
                items.push({ module: '地图', moduleId: 'map', icon: 'map', id: loc.id || '-', name: loc.name || '未命名' });
            });
            ...
            if (Array.isArray(data.locations)) {
                ...
                for (const [id, loc] of Object.entries(data.locations)) {
                    items.push({ module: '地图', moduleId: 'map', icon: 'map', id: id, name: loc.name || '未命名' });
                }
            }
        }
        ...
        if (Array.isArray(v183.characters)) {
            v183.characters.forEach(c => {
                items.push({ module: '关系(人物)', moduleId: 'relation', icon: 'user_group', id: c.id || '-', name: c.name || '未命名' });
            });
        ...
        if (Array.isArray(rels)) {
            rels.forEach(r => {
                ...
                items.push({ module: '关系', moduleId: 'relation', icon: 'user_group', id: r.id || '-', name: label });
            });
        }
        ...
        if (customCatsArr.length > 0) {
            customCatsArr.forEach(cat => {
                items.push({ module: '自定义(分类)', moduleId: 'custom', icon: 'edit', id: cat.id || '-', name: cat.name || '未命名' });
                if (cat.items) {
                    cat.items.forEach(item => {
                        items.push({ module: '自定义(条目)', moduleId: 'custom', icon: 'edit', id: item.id || '-', name: item.name || '未命名' });
                    });
                }
            });
        }
        ...
        if (Array.isArray(cats) && cats.length > 0) {
            cats.forEach(cat => {
                items.push({ module: '物品分类', moduleId: 'item-library', icon: 'folder', id: cat.id || '-', name: cat.name || '未命名' });
            });
        }
    ...
    function renderTable() {
        ...
        if (filteredItems.length === 0) {
            ...
            filteredItems.forEach(item => {
                ...
                html += '<tr>';
                html += `<td>${SvgIconLib.renderAuto(item.icon, 16)} ${item.module}</td>`;
    ...
    ModuleRegistry.register({
        id: 'id_manager', name: 'ID管理', icon: 'hash', group: 'system', order: 3, hidden: true,
        dataKeys: [],
