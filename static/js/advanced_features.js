...
function showButtonSettings() {
    apiRequest('/api/buttons/config')
        .then(config => {
            // 如果没有配置，使用默认配置
            if (!config || !config.tabs) {
                config = JSON.parse(JSON.stringify(defaultButtonConfig));
            }
            ...
            let html = `
                ...
                <div class="modal-body">
                    <h4>顶部标签按钮</h4>
                    <div class="button-config-list" id="tab-buttons-list">
            `;
            
            // 按order排序
            const sortedTabs = [...config.tabs].sort((a, b) => a.order - b.order);
            
            sortedTabs.forEach((btn, index) => {
                html += `
                    <div class="button-config-item" data-id="${btn.id}">
                        <span class="btn-order">${index + 1}</span>
                        <div class="btn-info">
                            <input type="text" class="btn-name-input" value="${btn.name}" data-id="${btn.id}">
                        </div>
                        <div class="btn-actions">
                            <button class="btn-small" onclick="moveTabButton('${btn.id}', -1)">↑</button>
                            <button class="btn-small" onclick="moveTabButton('${btn.id}', 1)">↓</button>
                        </div>
                    </div>
                `;
            ...
            html += `
                    ...
                    <div class="modal-actions">
                        <button class="btn-secondary" onclick="resetButtonConfig()">恢复默认</button>
                        <button class="btn-primary" onclick="saveButtonConfig()">保存设置</button>
                    </div>
                </div>
            `;
            ...
            // 保存当前配置到全局
            window.currentButtonConfig = config;
        })
        .catch(error => {
            showToast('加载失败');
            console.error(error);
...
// 移动标签按钮顺序
function moveTabButton(btnId, direction) {
    const config = window.currentButtonConfig || JSON.parse(JSON.stringify(defaultButtonConfig));
    const btn = config.tabs.find(b => b.id === btnId);
    if (!btn) return;
    
    const currentOrder = btn.order;
    const newOrder = currentOrder + direction;
    
    // 找到目标位置的按钮
    const targetBtn = config.tabs.find(b => b.order === newOrder);
    if (targetBtn) {
        targetBtn.order = currentOrder;
    }
    btn.order = newOrder;
...
function saveButtonConfig() {
    ...
    apiRequest('/api/buttons/config/save', 'POST', { config: config })
    .then(data => {
        if (data && data.success) {
            showToast('保存成功');
            closeModal();
...
function resetButtonConfig() {
    ...
    window.currentButtonConfig = JSON.parse(JSON.stringify(defaultButtonConfig));
    showButtonSettings();
}

// 应用按钮配置
function applyButtonConfig(config) {
    if (!config || !config.bottomNav) return;
    
    // 更新底部导航按钮
    const sortedNav = [...config.bottomNav].sort((a, b) => a.order - b.order);
    const navContainer = document.querySelector('.bottom-nav');
    
    if (navContainer) {
        sortedNav.forEach(navConfig => {
            const btn = document.querySelector(`.nav-btn[data-page="${navConfig.id}"]`);
            if (btn) {
                // 更新名称和图标
                const label = btn.querySelector('.nav-label');
                const icon = btn.querySelector('.nav-icon');
                if (label) label.textContent = navConfig.name;
                if (icon) icon.textContent = navConfig.icon;
                
                // 显示/隐藏
                if (navConfig.visible === false) {
        ...
        // 重新让当前按钮居中
        setTimeout(() => {
            scrollNavButtonToCenter(currentPage);
        }, 50);
    }
}
...
function loadAndApplyButtonConfig() {
    apiRequest('/api/buttons/config')
        .then(config => {
            if (config && config.tabs) {
                applyButtonConfig(config);
...
function switchPage(pageId) {
    ...
    if (bottomNav) {
        scrollNavButtonToCenter(pageId);
    }
...
// 让导航按钮居中显示
function scrollNavButtonToCenter(pageId) {
