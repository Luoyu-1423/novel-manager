...
(function() {
    ...
    // SVG 图标定义
    const svgIcons = {
        character: '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 4a4 4 0 1 1 0 8 4 4 0 0 1 0-8zm0 10c-4.42 0-8 1.79-8 4v2h16v-2c0-2.21-3.58-4-8-4z"/></svg>',
    ...
    function previewEquipment(data) {
        const ad = data || window.appData;
        if (!ad || !ad.equipmentSlots || ad.equipmentSlots.length === 0) return '<p>暂无装备槽位</p>';
        let html = '<ul style="margin-left:20px;">';
        ad.equipmentSlots.forEach(slot => {
    ...
    function exportEquipment(data) {
        const ad = data || window.appData;
        if (!ad || !ad.equipmentSlots || ad.equipmentSlots.length === 0) return '';
        let text = '=== 装备 ===\n\n';
        ad.equipmentSlots.forEach(slot => {
            const item = slot.item;
            if (item) {
    ...
    // --- 故事线组 ---
    ModuleRegistry.register({
        id: 'story', name: '剧情', icon: 'book', group: 'story',
        svgIcon: svgIcons.story, order: 1,
        dataKeys: ['story'],
    ...
    ModuleRegistry.register({
        id: 'map', name: '地图', icon: 'map', group: 'story',
        svgIcon: svgIcons.map, order: 2,
    ...
    ModuleRegistry.register({
        id: 'relation', name: '关系', icon: 'user_group', group: 'story',
        svgIcon: svgIcons.relation, order: 3,
    ...
    ModuleRegistry.register({
        id: 'custom', name: '自定义', icon: 'edit', group: 'story',
        svgIcon: svgIcons.custom, order: 4,
    ...
    ModuleRegistry.register({
        ...
        svgIcon: svgIcons.preview, order: 1,
        dataKeys: [],
